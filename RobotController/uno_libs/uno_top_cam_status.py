...
class UnoTopCamStatusCheck(ClockTimer):
    ...
    def __init__(self, stop_event, rlock, ws=None, config=None):
        ...
        self.top_cam_watch_dog = NetworkWatchDog(tmsconfig.TOP_CAM_IP,2.0,self.status_ok,self.status_fail)
        self.setTimeoutCallback(self.top_cam_watch_dog.have_ping)

        self.__config = config
        self.logger = self.__config.logger or logging.getLogger(__name__)

        self.__api_path = "status/topcam"
    ...
    def send_tms_msg(self,msg, msg_type=tmsconfig.MSG_TYPE_STATUS, add_date=True, store_log=True):
        try:
            ...
            self.logger.debug(str_now+": Sending top cam status message")
