...
from ..teks_libs.usb_watch_dog import *

from ..tms.pojo.tms_msgs import TMSMsgs, TMSMsgDatas, _UNO_ID

class UnoUsbWifiStatusCheck(ClockTimer):
    ...
    def __init__(self, stop_event, rlock, ws=None, config=None):
        ...
        self.usb_wifi_watch_dog = USBWatchDog(None,2.0,self.status_ok,self.status_fail,network_adaptor=tmsconfig.UNO_CONNECT_NET_ADAPTOR)
        self.setTimeoutCallback(self.usb_wifi_watch_dog.have_ping)

        self.__config = config
        self.logger = self.__config.logger or logging.getLogger(__name__)

        self.__api_path = "status/usbwifi"
    ...
    def send_tms_msg(self,msg, msg_type=tmsconfig.MSG_TYPE_STATUS, add_date=True, store_log=True):
        try:
            ...
            self.logger.debug(str_now+": Sending usb wifi status message")
